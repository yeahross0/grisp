...
pub fn op_eval_inner(
    ...
    'eval_loop: loop {
        ...
        let result = eval_ins(code, env.clone(), stack, &mut pc, &mut call_frames)
            .map_err(|e| create_runtime_error(e.msg, &call_frames))?;
        code = result.0;
        env = result.1;
        ...
        ) -> Result<(Rc<Vec<Opcode>>, Rc<RefCell<Env>>), RuntimeError> {
            let ins = &code[*pc];
            *pc += 1;
                ...
                Opcode::MakeFunction { arg_count } => {
                    let body_code = stack
                        ...
                        .ok_or_else(|| RuntimeError {
                            msg: "Interpreter stack error getting function body".to_owned(),
                        })?
                        .0;
                    let params = stack
                        .pop()
                        .ok_or_else(|| RuntimeError {
                            msg: "Interpreter stack error getting params".to_owned(),
                        })?
                        .0;
